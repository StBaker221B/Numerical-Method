...
int main()
{
    ...
    for(int k = 1; k < 40; k++)
    {
        double u = lamda1[0] + k * ( lamda1[1] - lamda1[0] )/40;
        double a2[N] = {0};
        for(int j = 0; j < n; j++)
        {
            a2[j] = a[j] - u;
        }
        lamda2[k-1] = antipowermethod(a2, b, c, x, &s, n);
        lamda2[k-1] += u;
        cout<<k<<" :  "<<lamda2[k-1]<<'\n';
    }

    // det
    // det = LUbound(a, b, c, x, n);
...
double antipowermethod(double* a, double b, double c, double* x, double* det, int n)
{
    ...
    for(; ; )
    {
        ...
        *det = LUbound(a, b, c, y, u, n);     // sol equation groups to avoid reverse matrix
        it++;
        // cout<<it<<'\n';
